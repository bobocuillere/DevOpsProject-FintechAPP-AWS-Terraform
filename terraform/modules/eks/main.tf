...
resource "aws_iam_policy" "eks_worker_ebs" {
  name        = "eks-worker-ebs"
  path        = "/"
  description = "EKS Worker Node policy for EBS Volume management"
...
resource "aws_iam_role_policy_attachment" "eks_worker_attach_ebs" {
  policy_arn = aws_iam_policy.eks_worker_ebs.arn
  role       = aws_iam_role.eks_node_role.name
}

resource "aws_iam_role_policy_attachment" "eks__attach_ebs" {
  policy_arn = aws_iam_policy.eks_worker_ebs.arn
  role       = aws_iam_role.eks_cluster_role.name
}